...
export default function DriverNotificationsPage() {
  ...
  useEffect(() => {
    load()
    const supabase = createClient()
    const ch = supabase.channel('driver-notifications').on('postgres_changes', { event: '*', schema: 'public', table: 'notifications' }, load).subscribe()
            ...
            <div className="divide-y divide-slate-50">
              {notifications.map((n) => (
                <button key={n.id} onClick={() => !n.is_read && markRead(n.id)}
                  className={`w-full flex items-start gap-4 px-6 py-4 text-left transition-colors ${n.is_read ? 'hover:bg-slate-50/40' : 'bg-emerald-50/60 hover:bg-emerald-50'}`}
                >
                  <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 text-base ${n.is_read ? 'bg-slate-100' : 'bg-emerald-100'}`}>
