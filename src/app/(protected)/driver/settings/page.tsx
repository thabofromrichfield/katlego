...
function TextInput({ value, onChange, placeholder, type = 'text', disabled }: {
  ...
  return (
    <input
      ...
      onFocus={e => { if (!disabled) e.target.style.borderColor = '#059669' }}
...
function CardHead({ title, sub }: { title: string; sub: string }) {
  return <div style={{ padding: '20px 24px 16px', borderBottom: '1px solid #f1f5f9' }}><p style={{ fontSize: 15, fontWeight: 700, color: '#0f172a' }}>{title}</p><p style={{ fontSize: 13, color: '#64748b', marginTop: 2 }}>{sub}</p></div>
}
  ...
  if (!profile) {
    return (
      <div style={{ maxWidth: 600 }}>
        {[...Array(2)].map((_, i) => <div key={i} style={{ height: 120, background: '#f1f5f9', borderRadius: 16, marginBottom: 20 }} />)}
      </div>
    )
  }
  ...
  return (
    <div style={{ maxWidth: 600 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, background: 'white', borderRadius: 16, border: '1px solid #e2e8f0', boxShadow: '0 1px 4px rgba(15,23,42,0.06)', padding: 20, marginBottom: 20 }}>
        <div style={{ width: 64, height: 64, borderRadius: 16, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: 20, fontWeight: 800, flexShrink: 0, background: 'linear-gradient(135deg,#059669,#047857)', boxShadow: '0 4px 16px rgba(5,150,105,0.3)' }}>
          {generateInitials(fullName || profile.full_name)}
        </div>
        <div>
          <p style={{ fontSize: 18, fontWeight: 800, color: '#0f172a' }}>{profile.full_name}</p>
          <p style={{ fontSize: 13, color: '#64748b', marginTop: 4, display: 'flex', alignItems: 'center', gap: 6 }}>
            <Shield style={{ width: 14, height: 14, color: '#059669' }} /> Driver
          </p>
        </div>
      </div>

      <Card>
        <CardHead title="Profile" sub="Your public information" />
        <CardBody>
          ...
          <div>
            <button
              ...
              style={{ display: 'inline-flex', alignItems: 'center', gap: 8, height: 40, padding: '0 20px', background: '#059669', color: 'white', border: 'none', borderRadius: 12, fontSize: 14, fontWeight: 700, cursor: 'pointer', opacity: savingProfile ? 0.6 : 1 }}
            >
      ...
      <Card>
        <CardHead title="Password" sub="Change your account password" />
        <CardBody>
